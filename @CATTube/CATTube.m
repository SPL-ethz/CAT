classdef CATTube < CAT
    
    %% Crystallization Analysis Toolbox GUI (CATTube)
    %
    %%% Usage
    %
    % CAT uses an object-oriented approach. To use the CAT GUI, simply run:
    %	>> C = CATTube;
    %
    % SEE ALSO
    % CAT, Distribution
    
    %% CATTube properties
    
    properties (Access=protected)
        
        % GUI handles and stuff
        gui
        
    end % properties
    
    %% CATTube methods
    
    methods
        
        %% - Constructor method
        
        function O = CATTube(O_original)
                        
            % Start constructing GUI
            
            % Initialise the main GUI, define all buttons and graphics
            % objects
            
            %
            % Initialise the GUI
            %
            
            if nargin < 1
                % The original is an empty CAT object
                O_original = CAT;
            end
            
            % Create the figure handle
            O.gui.fighandle = figure(...
                'MenuBar','none',...
                'Name','Looking at CATs',...
                'NumberTitle','off',...
                'Position',[200 200 1050 445],...
                'Resize','off');
            
            % Create source structure which keeps list of which variable
            % was imported into each property
            O.gui.source = struct;
            
            % Create GUI ui controls
            
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % Initial conditions
            %
            
            % Panel for initial conditions
            O.gui.init.panel = uipanel('Parent',O.gui.fighandle,...
                'Title','Initial settings',...
                'Units','pixels',...
                'Position',[20 300 500 125]);
            
            % Distribution
            % Text
            O.gui.init.init_dist_text = uicontrol(O.gui.init.panel,...
                'Style','text',...
                'String','Distribution function',...
                'TooltipString',help('CAT.init_dist'),...
                'Position',[10 85 240 20]);
            O.gui.init.init_dist_warning = uicontrol(O.gui.init.panel,...
                'Style','text',...
                'String','!',...
                'ForegroundColor','r',...
                'visible','off',...
                'Position',[235 85 10 20]);
            % Input box
            O.gui.init.init_dist = uicontrol(O.gui.init.panel,...
                'Style','edit',...
                'Background','w',...
                'Enable','off',... It's not easy to define a distribution by typing, show only summary here
                'String','',...
                'TooltipString',help('CAT.init_dist'),...
                'Position',[250 85 140 20]);
            % Define button
            O.gui.init.init_dist_define = uicontrol(O.gui.init.panel,...
                'Style','pushbutton',...
                'String','Edit',...
                'Value',0,...
                'Callback',@(hObject,eventdata) O.editDist(hObject,eventdata),...
                'Position',[390 85 50 20]...
                );
            % Browse button
            O.gui.init.init_dist_browse = uicontrol(O.gui.init.panel,...
                'Style','pushbutton',...
                'String','Browse',...
                'Value',0,...
                'Callback',@(hObject,eventdata)O.CATbrowse(hObject,eventdata,'init_dist','Distribution'),...
                'Position',[440 85 50 20]...
                );
            
            % Seed mass
            % Text
            O.gui.init.init_seed_text = uicontrol(O.gui.init.panel,...
                'Style','text',...
                'String','Seed mass',...
                'TooltipString',help('CAT.init_seed'),...
                'Position',[10 60 240 20]);
            % Warning Sign
            O.gui.init.init_seed_warning = uicontrol(O.gui.init.panel,...
                'Style','text',...
                'String','!',...
                'visible','off',...
                'ForegroundColor','r',...
                'Position',[235 60 10 20]);
            % Input box
            O.gui.init.init_seed = uicontrol(O.gui.init.panel,...
                'Style','edit',...
                'String',data2str(O.init_seed),...
                'Background','w',...
                'TooltipString',help('CAT.init_seed'),...
                'Callback',@(hObject,eventdata)set(O,'init_seed',get(hObject,'String')),...
                'Position',[250 60 190 20]); %#ok<*ST2NM>
            % Browse button
            O.gui.init.init_seed_browse = uicontrol(O.gui.init.panel,...
                'Style','pushbutton',...
                'String','Browse',...
                'Value',0,...
                'Callback',@(hObject,eventdata)O.CATbrowse(hObject,eventdata,'init_seed','double'),...
                'Position',[440 60 50 20]...
                );
            
            % Mass medium (solvent + antisolvent?)
            % Text
            O.gui.init.init_massmedium_text = uicontrol(O.gui.init.panel,...
                'Style','text',...
                'String','Solvent mass',...
                'TooltipString',help('CAT.init_massmedium'),...
                'Position',[10 35 240 20]);
            % Warning Sign
            O.gui.init.init_massmedium_warning = uicontrol(O.gui.init.panel,...
                'Style','text',...
                'String','!',...
                'visible','off',...
                'ForegroundColor','r',...
                'Position',[235 35 10 20]);
            % Input box
            O.gui.init.init_massmedium = uicontrol(O.gui.init.panel,...
                'Style','edit',...
                'String',data2str(get(O,'init_massmedium')),...
                'Background','w',...
                'TooltipString',help('CAT.init_massmedium'),...
                'Callback',@(hObject,eventdata)set(O,'init_massmedium',get(hObject,'String')),...
                'Position',[250 35 190 20]);
            % Browse button
            O.gui.init.init_massmedium_browse = uicontrol(O.gui.init.panel,...
                'Style','pushbutton',...
                'String','Browse',...
                'Value',0,...
                'Callback',@(hObject,eventdata)O.CATbrowse(hObject,eventdata,'init_massmedium','double'),...
                'Position',[440 35 50 20]...
                );
            
            % c0
            % Text
            O.gui.init.init_conc_text = uicontrol(O.gui.init.panel,...
                'Style','text',...
                'String','Concentration',...
                'TooltipString',help('CAT.init_conc'),...
                'Position',[10 10 240 20]);
            % Warning Sign
            O.gui.init.init_conc_warning = uicontrol(O.gui.init.panel,...
                'Style','text',...
                'String','!',...
                'visible','off',...
                'ForegroundColor','r',...
                'Position',[235 10 10 20]);
            % Input box
            O.gui.init.init_conc = uicontrol(O.gui.init.panel,...
                'Style','edit',...
                'String',data2str(O.init_conc),...
                'Background','w',...
                'TooltipString',help('CAT.init_conc'),...
                'Callback',@(hObject,eventdata)set(O,'init_conc',get(hObject,'String')),...
                'Position',[250 10 190 20]);
            % Browse button
            O.gui.init.init_conc_browse = uicontrol(O.gui.init.panel,...
                'Style','pushbutton',...
                'String','Browse',...
                'Value',0,...
                'Callback',@(hObject,eventdata)O.CATbrowse(hObject,eventdata,'init_conc','double'),...
                'Position',[440 10 50 20]...
                );
            
            
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % Process
            %
            
            % Panel for process settings
            O.gui.proc.panel = uipanel('Parent',O.gui.fighandle,...
                'Title','Process settings',...
                'Units','pixels',...
                'Position',[20 215 500 75]);
            
            % Tprofile
            % Text
            O.gui.proc.Tprofile_text = uicontrol(O.gui.proc.panel,...
                'Style','text',...
                'String','Temperature Profile',...
                'ToolTipString',help('CAT.Tprofile'),...
                'Position',[10 35 240 20]);
            % Warning Sign
            O.gui.proc.Tprofile_warning = uicontrol(O.gui.proc.panel,...
                'Style','text',...
                'String','!',...
                'visible','off',...
                'ForegroundColor','r',...
                'Position',[235 35 10 20]);
            % Input box
            O.gui.proc.Tprofile = uicontrol(O.gui.proc.panel,...
                'Style','edit',...
                'String',data2str(O.Tprofile),...
                'Background','w',...
                'ToolTipString',help('CAT.Tprofile'),...
                'Callback',@(hObject,eventdata)set(O,'Tprofile',get(hObject,'String')),...
                'Position',[250 35 190 20]);
            % Browse button
            O.gui.proc.Tprofile_browse = uicontrol(O.gui.proc.panel,...
                'Style','pushbutton',...
                'String','Browse',...
                'Value',0,...
                'Callback',@(hObject,eventdata)O.CATbrowse(hObject,eventdata,'Tprofile',{'double','function_handle'}),...
                'Position',[440 35 50 20]...
                );
            
            % ASprofile
            % Text
            O.gui.proc.ASprofile_text = uicontrol(O.gui.proc.panel,...
                'Style','text',...
                'String','Antisolvent Profile',...
                'ToolTipString',help('CAT.ASprofile'),...
                'Position',[10 10 240 20]);
            % Warning Sign
            O.gui.proc.ASprofile_warning = uicontrol(O.gui.proc.panel,...
                'Style','text',...
                'String','!',...
                'visible','off',...
                'ForegroundColor','r',...
                'Position',[235 10 10 20]);
            % Input box
            O.gui.proc.ASprofile = uicontrol(O.gui.proc.panel,...
                'Style','edit',...
                'String',data2str(O.ASprofile),...
                'Background','w',...
                'ToolTipString',help('CAT.ASprofile'),...
                'Callback',@(hObject,eventdata)set(O,'ASprofile',get(hObject,'String')),...
                'Position',[250 10 190 20]);
            % Browse button
            O.gui.proc.ASprofile_browse = uicontrol(O.gui.proc.panel,...
                'Style','pushbutton',...
                'String','Browse',...
                'Value',0,...
                'Callback',@(hObject,eventdata)O.CATbrowse(hObject,eventdata,'ASprofile',{'double','function_handle'}),...
                'Position',[440 10 50 20]...
                );
            
            
            
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % Thermodynamics
            %
            
            % Panel for thermodynamics settings
            O.gui.td.panel = uipanel('Parent',O.gui.fighandle,...
                'Title','Thermodynamics and substance properties',...
                'Units','pixels',...
                'Position',[20 105 500 100]);
            
            % Solubility
            O.gui.td.solubility_text = uicontrol(O.gui.td.panel,...
                'Style','text',...
                'String','Solubility function',...
                'TooltipString',help('CAT.solubility'),...
                'Position',[10 60 240 20]);
            % Warning Sign
            O.gui.td.solubility_warning = uicontrol(O.gui.td.panel,...
                'Style','text',...
                'String','!',...
                'visible','off',...
                'ForegroundColor','r',...
                'Position',[235 60 10 20]);
            % Input box
            O.gui.td.solubility = uicontrol(O.gui.td.panel,...
                'Style','edit',...
                'String',data2str(O.solubility),...
                'Background','w',...
                'TooltipString',help('CAT.solubility'),...
                'Callback',@(hObject,eventdata)set(O,'solubility',get(hObject,'String')),...
                'Position',[250 60 190 20]);
            % Browse button
            O.gui.td.solubility_browse = uicontrol(O.gui.td.panel,...
                'Style','pushbutton',...
                'String','Browse',...
                'Value',0,...
                'Callback',@(hObject,eventdata)O.CATbrowse(hObject,eventdata,'solubility',{'double','function_handle'}),...
                'Position',[440 60 50 20]...
                );
            
            % rhoc
            O.gui.td.rhoc_text = uicontrol(O.gui.td.panel,...
                'Style','text',...
                'String','Crystal density',...
                'ToolTipString',help('CAT.rhoc'),...
                'Position',[10 35 240 20]);
            O.gui.td.rhoc_warning = uicontrol(O.gui.td.panel,...
                'Style','text',...
                'String','!',...
                'visible','off',...
                'ForegroundColor','r',...
                'Position',[235 35 10 20]);
            % Input box
            O.gui.td.rhoc = uicontrol(O.gui.td.panel,...
                'Style','edit',...
                'String',data2str(O.rhoc),...
                'Background','w',...
                'ToolTipString',help('CAT.rhoc'),...
                'Callback',@(hObject,eventdata)set(O,'rhoc',get(hObject,'String')),...
                'Position',[250 35 190 20]);
            % Browse button
            O.gui.td.rhoc_browse = uicontrol(O.gui.td.panel,...
                'Style','pushbutton',...
                'String','Browse',...
                'Value',0,...
                'Callback',@(hObject,eventdata)O.CATbrowse(hObject,eventdata,'rhoc','double'),...
                'Position',[440 35 50 20]...
                );
            
            % kv
            O.gui.td.kv_text = uicontrol(O.gui.td.panel,...
                'Style','text',...
                'String','Shape factor',...
                'ToolTipString',help('CAT.kv'),...
                'Position',[10 10 240 20]);
            % Warning Sign
            O.gui.td.kv_warning = uicontrol(O.gui.td.panel,...
                'Style','text',...
                'String','!',...
                'visible','off',...
                'ForegroundColor','r',...
                'Position',[235 10 10 20]);
            % Input box
            O.gui.td.kv = uicontrol(O.gui.td.panel,...
                'Style','edit',...
                'String',data2str(O.kv),...
                'Background','w',...
                'ToolTipString',help('CAT.kv'),...
                'Callback',@(hObject,eventdata)set(O,'kv',get(hObject,'String')),...
                'Position',[250 10 190 20]);
            % Browse button
            O.gui.td.kv_browse = uicontrol(O.gui.td.panel,...
                'Style','pushbutton',...
                'String','Browse',...
                'Value',0,...
                'Callback',@(hObject,eventdata)O.CATbrowse(hObject,eventdata,'kv','double'),...
                'Position',[440 10 50 20]...
                );
            
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % Kinetics
            %
            
            % Panel for kinetics settings
            O.gui.kin.panel = uipanel('Parent',O.gui.fighandle,...
                'Title','Kinetics',...
                'Units','pixels',...
                'Position',[20 20 500 75]);
            
            % Growth rate
            O.gui.kin.growthrate_text = uicontrol(O.gui.kin.panel,...
                'Style','text',...
                'String','Growth rate',...
                'ToolTipString',help('CAT.growthrate'),...
                'Position',[10 35 240 20]);
            % Warning Sign
            O.gui.kin.growthrate_warning = uicontrol(O.gui.kin.panel,...
                'Style','text',...
                'String','!',...
                'visible','off',...
                'ForegroundColor','r',...
                'Position',[235 35 10 20]);
            % Input box
            O.gui.kin.growthrate = uicontrol(O.gui.kin.panel,...
                'Style','edit',...
                'String',data2str(O.growthrate),...
                'Background','w',...
                'ToolTipString',help('CAT.growthrate'),...
                'Callback',@(hObject,eventdata)set(O,'growthrate',get(hObject,'String')),...
                'Position',[250 35 190 20]);
            % Browse button
            O.gui.kin.growthrate_browse = uicontrol(O.gui.kin.panel,...
                'Style','pushbutton',...
                'String','Browse',...
                'Value',0,...
                'Callback',@(hObject,eventdata)O.CATbrowse(hObject,eventdata,'growthrate',{'double','function_handle'}),...
                'Position',[440 35 50 20]...
                );
            
            % Nucleation rate
            O.gui.kin.nucleationrate_text = uicontrol(O.gui.kin.panel,...
                'Style','text',...
                'String','Nucleation rate',...
                'ToolTipString',help('CAT.nucleationrate'),...
                'Position',[10 10 240 20]);
            % Input box
            O.gui.kin.nucleationrate = uicontrol(O.gui.kin.panel,...
                'Style','edit',...
                'String',data2str(O.nucleationrate),...
                'Background','w',...
                'ToolTipString',help('CAT.nucleationrate'),...
                'Callback',@(hObject,eventdata)set(O,'nucleationrate',get(hObject,'String')),...
                'Position',[250 10 190 20]);
            % Browse button
            O.gui.kin.nucleationrate_browse = uicontrol(O.gui.kin.panel,...
                'Style','pushbutton',...
                'String','Browse',...
                'Value',0,...
                'Callback',@(hObject,eventdata)O.CATbrowse(hObject,eventdata,'nucleationrate',{'double','function_handle'}),...
                'Position',[440 10 50 20]...
                );
            
            
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % Solver
            %
            
            % Panel for solver settings
            O.gui.solv.panel = uipanel('Parent',O.gui.fighandle,...
                'Title','Solver settings',...
                'Units','pixels',...
                'Position',[530 325 500 100]);
            
            % Time
            O.gui.solv.sol_time_text = uicontrol(O.gui.solv.panel,...
                'Style','text',...
                'String','Time',...
                'ToolTipString',help('CAT.sol_time'),...
                'Position',[10 60 240 20]);
            O.gui.solv.sol_time_warning = uicontrol(O.gui.solv.panel,...
                'Style','text',...
                'String','!',...
                'visible','off',...
                'ForegroundColor','r',...
                'Position',[235 60 10 20]);
            % Input box
            O.gui.solv.sol_time = uicontrol(O.gui.solv.panel,...
                'Style','edit',...
                'String','',...
                'ToolTipString',help('CAT.sol_time'),...
                'Background','w',...
                'Callback',@(hObject,eventdata)set(O,'sol_time',get(hObject,'String')),...
                'Position',[250 60 190 20]);
            % Browse button
            O.gui.solv.sol_time_browse = uicontrol(O.gui.solv.panel,...
                'Style','pushbutton',...
                'String','Browse',...
                'Value',0,...
                'Callback',@(hObject,eventdata)O.CATbrowse(hObject,eventdata,'sol_time','double'),...
                'Position',[440 60 50 20]...
                );
            
            % Solution method
            O.gui.solv.sol_method_text = uicontrol(O.gui.solv.panel,...
                'Style','text',...
                'String','Solution method',...
                'ToolTipString',help('CAT.sol_method'),...
                'Position',[10 35 240 20]);
            % List box for choices
            O.gui.solv.sol_method = uicontrol(O.gui.solv.panel,...
                'Style','popup',...
                'String','Central difference|Moving pivot|Hires',...
                'ToolTipString',help('CAT.sol_method'),...
                'Callback',@(hObject,eventdata) set(O,'sol_method',O.parenthesis(get(hObject,'String'),get(hObject,'Value'),':')),...
                'Position',[250 35 240 20]);
            
            % Options
            O.gui.solv.sol_options_text = uicontrol(O.gui.solv.panel,...
                'Style','text',...
                'String','Options',...
                'Position',[10 10 240 20]);
            % Input box
            O.gui.solv.sol_options = uicontrol(O.gui.solv.panel,...
                'Style','edit',...
                'String','',...
                'TooltipString',help('CAT.sol_options'),...
                'Position',[250 10 190 20]);
            % Browse button
            O.gui.solv.sol_options_browse = uicontrol(O.gui.solv.panel,...
                'Style','pushbutton',...
                'String','Browse',...
                'Value',0,...
                'Callback',@(hObject,eventdata)O.CATbrowse(hObject,eventdata,'sol_options','cell'),...
                'Position',[440 10 50 20]...
                );

            
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % Run panel
            %
            
            % Panel for running programs
            O.gui.run.panel = uipanel('Parent',O.gui.fighandle,...
                'Title','Run',...
                'Units','pixels',...
                'Position',[530 20 500 295]);
            
            % Reset button
            O.gui.run.reset = uicontrol(O.gui.run.panel,...
                'Style','pushbutton',...
                'String','Reset Values',...
                'Value',0,...
                'callback',@(hObject,Event) O.load(O_original),...
                'Position',[20 220 225 30]...
                );
            
            
            % Default button
            O.gui.run.default = uicontrol(O.gui.run.panel,...
                'Style','pushbutton',...
                'String','Default Values',...
                'Value',0,...
                'Callback',@(hObject,Event)O.setDefaults,...
                'Position',[245 220 225 30]...
                );
            
            % Solve button
            O.gui.run.start = uicontrol(O.gui.run.panel,...
                'Style','pushbutton',...
                'String','Solve',...
                'fontsize',10,...
                'fontweight','bold',...
                'Value',0,...
                'Enable','off',...
                'Callback',@(hObject,Event)O.solve,...
                'Position',[20 180 450 30]...
                );
            
            % Plot button
            O.gui.run.plot = uicontrol(O.gui.run.panel,...
                'Style','pushbutton',...
                'String','Plot results',...
                'Value',0,...
                'Enable','off',...
                'Callback',@(hObject,Event)O.plot,...
                'Position',[20 140 185 30]...
                );
            
            % Close Plots button
            O.gui.run.plot_close = uicontrol(O.gui.run.panel,...
                'Style','pushbutton',...
                'String','Close plots',...
                'Value',0,...
                'Enable','on',...
                'Callback',@(hObject,Event)O.plot('close'),...
                'Position',[205 140 185 30]...
                );
            
            % Overwrite Checkbox
            O.gui.run.plot_overwrite = uicontrol(O.gui.run.panel,...
                'Style','checkbox',...
                'String','Overwrite',...
                'Value',1,...
                'Enable','off',...
                'Position',[400 140 100 30]...
                );
            
            % Load CAT
            O.gui.run.import = uicontrol(O.gui.run.panel,...
                'Style','pushbutton',...
                'String','Load CAT from .mat',...
                'Value',0,...
                'Enable','on',...
                'Callback',@(hObject,Event) O.load,...
                'Position',[20 40 225 30]...
                );
            
            % Load CAT
            O.gui.run.browseCat = uicontrol(O.gui.run.panel,...
                'Style','pushbutton',...
                'String','Load CAT from Workspace',...
                'Value',0,...
                'Enable','on',...
                'Callback',@(hObject,eventdata)O.CATbrowse(hObject,eventdata,[],'CAT'),...
                'Position',[245 40 225 30]...
                );
            
            % Save CAT
            O.gui.run.export = uicontrol(O.gui.run.panel,...
                'Style','pushbutton',...
                'String','Save',...
                'Value',0,...
                'Enable','on',...
                'Callback',@(hObject,Event) O.save,...
                'Position',[20 10 225 30]...
                );
            
            % Load the data as defined in the input to the CATTube call
            O.load(O_original);
                
        end % function

        %% - editDist
        
        function editDist(O,~,~)
            if isempty(O.init_dist)
                O.init_dist = Distribution;
            end
                
            O.init_dist.edit;       
            
            set(O.gui.init.init_dist,'String',dist2str(O.init_dist));
            
        end % function
        
        %% - CATbrowse
        
        function CATbrowse(O,~,~,classvarname,classfilter)
            
            [vardata,varname] = browseVars(classfilter);
            
            if ~isempty(classvarname) && ~isempty(vardata)
                O.(classvarname) = vardata;
                O.gui.source.(classvarname) = varname;
            end % if
            
        end % function
        
        %% - plot
        
        function plot(O,varargin)
            
            % overwrite plots only if overwrite checkbox is ticked
            if ~get(O.gui.run.plot_overwrite,'value')
                varargin = {varargin{:},'add'};
            end
            
            plot@CAT(O,varargin{:}); % calling plot function of class CAT
            
        end % function
        
        %% - solvableCheck
        
        function validInput = solvableCheck(O,fieldnames)
            
            if ~iscell(fieldnames)
                fieldnames = {fieldnames};
            end
            validInput = zeros(size(fieldnames));
            i = 1;
            while i <= length(fieldnames) && ~any(validInput) 
                fieldname = fieldnames{i};
                if nargin>1
                    [validInput(i),solvableFlag] = O.diagnose(fieldname,O.(fieldname),'solvable');
                else
                    [~,solvableFlag] = O.diagnose([],[],'solvable');
                    validInput(i) = [];
                end

                if solvableFlag && isfield(O.gui,'run')
                    set(O.gui.run.start,'enable','on');
                elseif isfield(O.gui,'run')
                    set(O.gui.run.start,'enable','off');
                end
                i = i+1;
            end
            
            
        end % function
        
        function updateGUI(O)
           
            fieldnames = properties(O);
            fieldnames(strcmp(fieldnames,'gui')) = [];
            fieldnames(~cellfun(@isempty,strfind(fieldnames,'calc'))) = [];
            warning('off','all')
            for i = 1:length(fieldnames)
               eval(['O.',fieldnames{i},'_onset']); 
            end
            warning('on','all')
            
        end
        
    end % methods
    
    
    %% Hidden methods
    
    methods(Hidden)
       % Onset Methods go here. Their entire point is to update the GUI when changes occur, there is no need to use these functions independent of normal set actions 
       
       %% - init_dist_onset
        
        function init_dist_onset(O)
            
            % Update the init_dist field
            set(O.gui.init.init_dist,'String',strrep(dist2str(O.init_dist),'[]',''));
            validInput = O.solvableCheck('init_dist');

            if validInput
                set(O.gui.init.init_dist_warning,'visible','off')
            else
                set(O.gui.init.init_dist_warning,'visible','on')
            end
        end % function
        
        %% - init_seed_onset
        
        function init_seed_onset(O)
            
            % Update the init_seed field
            set(O.gui.init.init_seed,'String',strrep(data2str(O.init_seed),'[]',''));
            validInput = O.solvableCheck('init_seed');

            if validInput
                set(O.gui.init.init_seed_warning,'visible','off')
            else
                set(O.gui.init.init_seed_warning,'visible','on')
            end

        end % function
        
        %% - init_massmedium_onset
        
        function init_massmedium_onset(O)
            
            % Update the init_massmedium field
            set(O.gui.init.init_massmedium,'String',strrep(data2str(O.init_massmedium),'[]',''));
            validInput = O.solvableCheck('init_massmedium');
            
            if validInput
                set(O.gui.init.init_massmedium_warning,'visible','off')
            else
                set(O.gui.init.init_massmedium_warning,'visible','on')
            end
        end % function
        
        %% - init_conc_onset
        
        function init_conc_onset(O)
            
            % Update the init_conc field
            set(O.gui.init.init_conc,'String',strrep(data2str(O.init_conc),'[]',''));
            validInput = O.solvableCheck('init_conc');
            
            if validInput
                set(O.gui.init.init_conc_warning,'visible','off')
            else
                set(O.gui.init.init_conc_warning,'visible','on')
            end;    
        end % function
        
        %% - Tprofile_onset
        
        function Tprofile_onset(O)
            
            % Update the Tprofile field
            if isfield(O.gui,'init')
                set(O.gui.proc.Tprofile,'String',strrep(data2str(O.Tprofile),'[]',''));
            end
            validInput = O.solvableCheck({'Tprofile','ASprofile'});
            
            if any(validInput)
                set(O.gui.proc.Tprofile_warning,'visible','off')
                set(O.gui.proc.ASprofile_warning,'visible','off')
            else
                set(O.gui.proc.Tprofile_warning,'visible','on')
                set(O.gui.proc.ASprofile_warning,'visible','on')
            end
        end % function
        
        %% - ASprofile_onset
        
        function ASprofile_onset(O)
            
            % Update the ASprofile field
            if isfield(O.gui,'init')
                set(O.gui.proc.ASprofile,'String',strrep(data2str(O.ASprofile),'[]',''));
            end
            
            validInput = O.solvableCheck({'ASprofile' 'Tprofile'});
            
            if any(validInput)
                set(O.gui.proc.Tprofile_warning,'visible','off')
                set(O.gui.proc.ASprofile_warning,'visible','off')
            else
                set(O.gui.proc.Tprofile_warning,'visible','on')
                set(O.gui.proc.ASprofile_warning,'visible','on')
            end
            
        end % function
        
        %% - solubility_onset
        
        function solubility_onset(O)
            
            % Update the solubility field
            if isfield(O.gui,'init')
                set(O.gui.td.solubility,'String',strrep(data2str(O.solubility),'[]',''));
                    set(O.gui.init.init_conc,'String',strrep(data2str(O.init_conc),'[]',''));
            end
            validInput = O.solvableCheck('solubility');
            
            if validInput
                set(O.gui.td.solubility_warning,'visible','off')
            else
                set(O.gui.td.solubility_warning,'visible','on')
            end
        end % function
        
        %% - rhoc_onset
        
        function rhoc_onset(O)
            
            % Update the rhoc field
            if isfield(O.gui,'init')
                set(O.gui.td.rhoc,'String',strrep(data2str(O.rhoc),'[]',''));
            end
            validInput = O.solvableCheck('rhoc');
            
            if validInput
                set(O.gui.td.rhoc_warning,'visible','off')
            else
                set(O.gui.td.rhoc_warning,'visible','on')
            end
        end % function
        
        %% - kv_onset
        
        function kv_onset(O)
            
            % Update the kv field
            if isfield(O.gui,'init')
                set(O.gui.td.kv,'String',strrep(data2str(O.kv),'[]',''));
            end
            validInput = O.solvableCheck('kv');
            
            if validInput
                set(O.gui.td.kv_warning,'visible','off')
            else
                set(O.gui.td.kv_warning,'visible','on')
            end
        end % function
        
        %% - growthrate_onset
        
        function growthrate_onset(O)
            
            % Update the growthrate field

            set(O.gui.kin.growthrate,'String',strrep(data2str(O.growthrate),'[]',''));
            validInput = O.solvableCheck('growthrate');
            
            if validInput
                set(O.gui.kin.growthrate_warning,'visible','off')
            else
                set(O.gui.kin.growthrate_warning,'visible','on')
            end
        end % function
        
        %% - nucleationrate_onset
        
        function nucleationrate_onset(O)
            
            % Update the nucleationrate field
            set(O.gui.kin.nucleationrate,'String',strrep(data2str(O.nucleationrate),'[]',''));
            
            
        end % function
        
        %% - sol_time_onset
        
        function sol_time_onset(O)
            
            % Update the sol_time field
            set(O.gui.solv.sol_time,'String',strrep(data2str(O.sol_time),'[]',''));
            
            validInput = O.solvableCheck('sol_time');
            
            if validInput
                set(O.gui.solv.sol_time_warning,'visible','off')
            else
                set(O.gui.solv.sol_time_warning,'visible','on')
            end
            
        end % function
        
        %% - sol_method_onset
        
        function sol_method_onset(O)
            
            % Update the sol_method field
            if ~isempty(O.sol_method)
                switch O.sol_method
                    case 'centraldifference'
                        value = 1;
                    case 'movingpivot'
                        value = 2;
                    case 'hires'
                        value = 3;
                end % switch
            else
                value = 1;
            end
            
            if isfield(O.gui,'init')
                set(O.gui.solv.sol_method,'Value',value);
            end
            
%             O.solvableCheck('sol_method');
            
        end % function
        
        %% - sol_options_onset
        
        function sol_options_onset(O)
            
            % Update the sol_options field
            if isfield(O.gui,'init')
                set(O.gui.solv.sol_options,'String',strrep(data2str(O.sol_options),'[]',''));
            end
            
%             O.solvableCheck('sol_options');
            
        end % function
        
       
    end
    
    %% Static methods
    
    methods (Static)
        
        function out = parenthesis(x, varargin) 
            
            out = x(varargin{:});
            
        end % function paren
        
    end % methods Static
    
    %% Static, protected methods
    
    methods(Static,Access=protected)
        
        status = solveroutput(t,y,flag);
        
    end % methods
end % classdef