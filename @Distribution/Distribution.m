...
%          end
% %          for i = 1:length(Ld)
% %              Ilb = find(yCC<Ld(i),1,'last');
% %              
% %              if ~isempty(Ilb)
% %              
% %                 NCC(Ilb) = NCC(Ilb) + N(i)*(Ld(i)^3-yCC(Ilb+1)^3)/(yCC(Ilb)^3-yCC(Ilb+1)^3);
% %                 NCC(Ilb+1) = NCC(Ilb+1) + N(i)*(1-(Ld(i)^3-yCC(Ilb+1)^3)/(yCC(Ilb)^3-yCC(Ilb+1)^3));
% %              end
% %          end
%          
%          I = find(NCC==0);
...
%          i = 1;
%         while i <= length(I)
%             % find consecutive sets
%             icount = 1;
% 
%             while i+icount<=length(I) && I(i+icount)==I(i)+icount
%                 icount = icount +1;
%             end
%             conSet{end+1} = [I(i):I(i)+icount-1];
%             i = i+ icount;
%             iset = iset + 1;
%         end
%         
...
%             
%         end
%         yCC = (boundariesCC(2:end)+boundariesCC(1:end-1))/2;
% 
%          
%          distCC = Distribution(yCC,NCC./diff(boundariesCC));
%          
%      end
