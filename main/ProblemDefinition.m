classdef ProblemDefinition < handle
    ...
    properties
        ...
        % Mass (of solvent + antisolvent) over time
        calc_massmedium
    ...
    methods
        ...
        function set.solubility(O,value)
            ...
            if isa(value,'function_handle') && nargin(value)==1
                ...
                O.solubility = @(T) value(T); % we assume the only input argument is Temperature
        ...
        function PDma = massbal(O)
            mass_solution = O.calc_conc(:).*O.calc_massmedium(:);   % total mass of solute
            m3 = moments(O.calc_dist,3);                            % third moment over time
            mass_crystals = O.rhoc*O.kv*O.calc_massmedium(:).*m3(:);    % total mass of crystals
            
            PDma = 100*(1-(mass_solution + mass_crystals)/(mass_solution(1)+mass_crystals(1)));     % mass balance error   
        end % function
...
%                 end % if
            ...
            % Handles for plots
                ...
                nopv = sum([~isempty(O.calc_conc)...
                    ~isempty(O.calc_temp) ~isempty(O.calc_massmedium)]);
                ...
                if nopv ~= 0
                    ...
                    if ~isempty(O.calc_massmedium)

                        subplot(1,nopv,3);                    
                        PDpl_local = plot(O.calc_time,O.calc_massmedium);
                        xlabel('Time')
